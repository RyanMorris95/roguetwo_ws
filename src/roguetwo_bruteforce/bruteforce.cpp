...
int main(int argc, char** argv){
	...
	while(ros::ok()){
		...
		if (initial){				//fix this, needs to relate to destination, not 0
			...
			goal = angle == 3.1 || angle == 0;	//1 = X-axis, 0 = Y-axis
		...
		if (finish) angle = atan2(X - destX, Y - destY);
		...
		angle = atan2(X - destX, Y - destY);
		if (angle >= 0) angle += M_PI/2;
		else angle -= M_PI/2;
		printf("orig-destXY: %lf\n", angle);

		angle = atan2(destX - X, destY - Y);
		if (angle >= 0) angle += M_PI/2;
		else angle -= M_PI/2;
		printf("dest-origXY: %lf\n", angle);

		angle = atan2(Y - destY, destX - X);
		//if (angle >= 0) angle += M_PI/2;
		//else angle -= M_PI/2;
		//printf("orig-destYX: %lf\n", angle);
		printf("tset: %lf\n", angle);
	
		angle = atan2(destY - Y, destX - X);
		if (angle >= 0) angle += M_PI/2;
		else angle -= M_PI/2;
		printf("dest-origYX: %lf\n", angle);
